...
|객체이름|type|설명|용도|
|---|---|---|---|
|request|javax.servlet.http.HttpServletRequest|클라이언트의 request 저장|페이지 입출력|
|response|javax.servlet.http.HttpServletResponse|서버의 response 저장|페이지 입출력|
|out|javax.servlet.jsp.JspWriter|페이지 내용 출력 스트림 객체|페이지 입출력|
|page|java.lang.Object|페이지가 구현된 자바 클래스 인스턴스|jsp 페이지 구현 서블릿|
|config|javax.servlet.ServletConfig|페이지 설정(config) 정보 |jsp 페이지 구현 서블릿|
|pageContext|javax.servlet.jsp.pageContext|페이지에 담긴 콘텐츠||
|application|javax.servlet.ServletContext|웹 applicatio에 담긴 콘텐츠||
|session|javax.servlet.http.HttpSession|HTTP 세션 관련 정보||
